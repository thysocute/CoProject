...
if (empty($team_stmt)) {
  ...
  echo     'You are not currently involved in any task, please go to create task！';
  echo   '</td>';
  echo '</tr>';
  exit();
  ...
  if (!empty($results)) {
    ...
    foreach ($results as $value) {
      ...
      echo '  <td>';
      echo '    <a href="../enStatic/updateTaskList.php?task_id='.$value['taskId'].'&task_name='.$value['taskName'].'">Modify </a>';
      echo '    <a href="../php/deleteTeam.php?task_id='.$value['taskId'].'&account='.$account.'">Delete</a>';
      echo '    <a href="../enStatic/addUser.php?task_id='.$value['taskId'].'&team_name='.$value['taskName'].'">Add member</a>';
      echo '  </td>';
      echo '</tr>';
      $index++;
    }
